...
def handle_receiving_RIP_distance_vector_packet(node: Node, recvPkt: Packet):
    ...
    try:
        ...
        for i in range(len(recvPkt.payload)):
            ...
            recvEntry = RIP_RoutingTableEntry(recvPkt.payload[i]['dest'], recvPkt.payload[i]['nextHop'],
                                              recvPkt.payload[i]['hopsToDest']);
...
def start_check_neighbor_nodes_alive_periodcally_thread(node: Node):
    t = threading.Thread(target=check_neighbor_nodes_alive_periodcally, args=(node,),
                         name='CheckNeighborNodesAliveThread');
    t.start();
